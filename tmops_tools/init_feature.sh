...
## Instance Prompts Location
- Chat prompt: docs/tmops_docs_v5/tmops_claude_chat.md
- Code prompts: docs/tmops_docs_v5/tmops_claude_code.md
EOF
fi

# Summary
echo ""
echo -e "${GREEN}=== Feature Initialization Complete ===${NC}"
echo ""
...
if [ "$RUN_TYPE" = "initial" ]; then
    ...
    echo "3. Provide each instance with its prompt from docs/tmops_docs_v5/"
else
    echo -e "${YELLOW}Patch run created with context from previous run${NC}"
fi

echo ""
echo -e "${GREEN}Ready to start TeamOps workflow!${NC}"